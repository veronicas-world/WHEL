...
**B7 · External validation layer (planned, deliberately outside the core).** Beyond Open Targets (which we *build* from), the field has established open knowledge graphs and models we can check signals against without importing them into our own graph: **DRKG** (Drug Repurposing Knowledge Graph), **PrimeKG** (Precision Medicine Knowledge Graph, Harvard/Zitnik), and **TxGNN** (zero-shot graph foundation model, Nature Medicine 2024). These stay an *outside cross-reference shown beside a signal*, not a build source, because they are trained on the same general biomedical record that under-covers women's hormonal and reproductive health — folding them in would import the exact male-default blind spot Whel exists to correct. Where they are silent on a women's-health pair, that silence is itself a finding. Use them as (a) a second/third "graph supports / silent / disagrees" cross-reference alongside Open Targets, and (b) a benchmark for our own surfacing (TxGNN predictions we validate, not adopt). This is the *validation-layer* track, distinct from the technical-architecture (build-source) track. Both tracks are now reflected publicly: the Roadmap page splits "Technical architecture" from "Validation layer," and the External References page lists these three as Planned with a "not integrated into the core architecture" note. No migration yet; this is a future move.
...
- **M6 (validation layer, later):** add the external open-KG/model cross-references (DRKG, PrimeKG, TxGNN) as an outside validation track shown beside signals, kept separate from the core architecture (B7).
