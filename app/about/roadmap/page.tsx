...
const REGISTER: { name: string; role: string; status: "Live" | "Under review" | "Planned" }[] = [
  ...
  { name: "Citation validation and summary grounding (Path C)", role: "Three-part output validation pipeline addressing LLM failure modes documented in the medical-LLM literature (Bhattacharyya et al. 2023, Cureus, doi:10.7759/cureus.39238 — 47 percent of ChatGPT-generated medical references fully fabricated, 46 percent authentic but inaccurate; Gravel et al. 2023, Mayo Clin Proc Digit Health, doi:10.1016/j.mcpdig.2023.05.004). Phase 1 (live as of June 7, 2026): pre-verified reference list at lib/whel-citations.json with scripts/verify-citations.py resolving every PMID against NCBI E-utilities, every DOI against the Crossref REST API, and every arXiv ID against the arXiv API. Also live: scripts/verify-database-sources.py auditing the 2,166-row live sources table (PMIDs against NCBI, NCT IDs against ClinicalTrials.gov, Open Targets IDs against Open Targets GraphQL, FAERS and Reddit URLs against format checks). First run on June 7, 2026: 170 resolved_match, 1,986 format_only_pass, 10 unresolved (all in the OT-DRUGNAME-shorthand finding tracked separately on this Roadmap). --strict mode exits non-zero on any unresolved or mismatched entry for pre-publish CI use; live audit numbers surfaced on /about/external-references 01d. Phase 2 (planned): sentence-level summary grounding using a sentence-transformer model (Sentence-BERT or equivalent) to compute cosine similarity between each summary sentence and the source abstract; flag sentences below the calibrated threshold. Phase 3 (planned): prompt hardening for published prose that forbids citation generation outside the Phase 1 manifest. Distinct from Path A and Path B (which ground LLM inputs); Path C validates LLM outputs. Recorded in methodology v3.6 (definition), v3.7 (manual audit prototype), v3.8 (Phase 1 live for manifest), v3.9 (Phase 1 tooling for database sources), and v3.10 (Phase 1 first database-sources run).", status: "Live" },
  { name: "Backfill canonical Open Targets identifiers on signals using OT-DRUGNAME shorthand", role: "Phase 1 first database-sources audit run (v3.10) caught 10 active signals where the sources.external_id column stores a synthetic OT-{DRUGNAME} string instead of a canonical Open Targets identifier. Affected drugs: APREPITANT, DESVENLAFAXINE, ENZALUTAMIDE, TRIMEBUTINE, TASIMELTEON, TRADIPITANT, OLAPARIB, FOSNETUPITANT, MILNACIPRAN, TRIIODOTHYRONINE (liothyronine). The url column on those rows already points at a real platform.opentargets.org page (with a real GO / EFO disease ontology ID), so users see a valid citation. Backfill: replace each OT-DRUGNAME with the canonical CHEMBL ID for the drug, plus the specific target ENSEMBL ID and disease MONDO/EFO ID encoded in the existing URL. Reduces the unresolved count on the database-sources audit to zero without changing what is rendered on /conditions/[slug].", status: "Planned" },
