  ...
  return [
    ...
    },
    {
      ...
      items: [
        ...
        "Add entity validation against canonical biomedical ontologies (ChEMBL or DrugBank, MONDO, HGNC) so LLM-extracted entities that fail to resolve are flagged for review rather than silently stored",
        "Stand up a domain-restricted knowledge graph cross-reference via BioCypher, displayed as 'graph supports' or 'graph silent' beside each signal in the same shape as the existing MATRIX disclosure",
...
const REGISTER: { name: string; role: string; status: "Live" | "Under review" | "Planned" }[] = [
  ...
  { name: "Entity validation against biomedical ontologies (Path A)", role: "Resolve every LLM-extracted compound, condition, and gene against canonical ontologies (ChEMBL or DrugBank, MONDO, HGNC) before writing to the database. Entities that fail to resolve are flagged for human review rather than silently stored. Closes the structured-output hallucination class of error documented in the LLM-fabrication literature (Gong et al. 2026). Recorded in methodology v3.4.", status: "Planned" },
  { name: "Knowledge graph cross-reference via BioCypher (Path B)", role: "Build a domain-restricted biomedical knowledge graph covering Whel's six conditions and active-signal compounds using the BioCypher framework (Lobentanzer et al., Nature Biotechnology 2023). Display 'graph supports' or 'graph silent' beside each signal in the same disclosure shape as the existing MATRIX cross-reference. Adds a structured-external-knowledge validation layer on top of LLM extraction without replacing it. Recorded in methodology v3.4.", status: "Planned" },
